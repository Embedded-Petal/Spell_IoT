...
class WebSocketsClient : protected WebSockets {
  public:
    void dockerBegin(const char* namespaceName);
    void dockerSaveString(const char* key, String value);
    String dockerReadString(const char* key, String defaultValue = "");

    void dockerSaveInt(const char* key, int value);
    int dockerReadInt(const char* key, int defaultValue = 0);
