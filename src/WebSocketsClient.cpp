...
void WebSocketsClient::dockerBegin(const char* namespaceName) {
    docker.begin(namespaceName, false);   // false = read/write
}

void WebSocketsClient::dockerSaveString(const char* key, String value) {
    docker.putString(key, value);
}

String WebSocketsClient::dockerReadString(const char* key, String defaultValue) {
    return docker.getString(key, defaultValue);
}

void WebSocketsClient::dockerSaveInt(const char* key, int value) {
    docker.putInt(key, value);
}

int WebSocketsClient::dockerReadInt(const char* key, int defaultValue) {
    return docker.getInt(key, defaultValue);
}

void WebSocketsClient::dockerEnd() {
    docker.end();
}
